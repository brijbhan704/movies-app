...
<div class="movies-info border-b border-gray-800">
	<div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
		<img src="{{'https://image.tmdb.org/t/p/w500/'.$movies['poster_path']}}" alt="parasite" class="w-64 md:w-96" style="width:24rem;">
		<div class="md:ml-24">
			<h2 class="text-4xl flex-wrap font-semibold">{{$movies['title']}}</h2>
                                    ...
                                    <div class="bg-gray-900 rounded">
                                        ...
                                        <div class="modal-body px-8 py-8">
                                            <div class="responsive-container overflow-hidden relative" style="padding-top: 56.25%">
                                                <iframe class="responsive-iframe absolute top-0 left-0 w-full h-full" src="https://www.youtube.com/embed/{{ $movies['videos']['results'][0]['key'] }}" style="border:0;" allow="autoplay; encrypted-media" allowfullscreen></iframe>
                                            </div>
                                        </div>
                                    </div>
                                </div>
	...
	<div class="container mx-auto px-4 py-16">
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">	
			@foreach($movies['credits']['cast'] as $cast)
				@if($loop->index < 5)
					<div class="mt-8">
						<a href="{{URL::to('actors_show',$cast['id'])}}">
						...
						<div class="mt-2">
							<a href="{{URL::to('actors_show',$cast['id'])}}" class="text-lg mt-2 hover:text-gray:300">{{$cast['name']}}</a>
							<div class="flex items-center text-gray-400  text-sm mt-1">
								<span class="mx-2">{{$cast['character']}}</span>			
							</div>				
						</div>
					</div>	
				@endif
